    ...
    with _open(fasta_file) as handle:
        for seq_record in SeqIO.parse(handle, "fasta"):
            ...
            if (
                ...
                int(sequence.count("N")) + int(sequence.count("-")) <= por_n
            ):
                pos_n = [m.end(0) for m in re.finditer("N", sequence)]
                check = [pos_n_i in mut_sites for pos_n_i in pos_n]
                pos_n1 = [m.end(0) for m in re.finditer("-", sequence)]
                check1 = [pos_n_i in mut_sites for pos_n_i in pos_n1]
                if sum(check) + sum(check1) < 10: # no N on mutsites of the interest
